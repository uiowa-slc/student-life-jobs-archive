...
class JobListingLocation extends JobListingCategorisationObject {
	...
	public function listingFeedURL() {
		return Environment::getEnv('JOBFEED_BASE') . 'feed/positions.json?location_id=' . $this->ID;
	}
	...
	public function JobListingsByFilter($status = 'open') {
		...
		$feedURL = Environment::getEnv('JOBFEED_BASE') . 'feed/positions.json?location_id=' . $this->ID . '&' . $term . '_id=' . $id;
