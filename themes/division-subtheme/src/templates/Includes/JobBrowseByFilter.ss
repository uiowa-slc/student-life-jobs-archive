  <div class="topic-browse-by-filter">
      ...
      <% if $Categories || $Departments %>
        ...
        <% loop $Categories.Sort('Title ASC') %>
            <div class="topic-browse-by-filter__item topic-browse-by-filter__item--small margin-bottom-1" data-equalizer-watch><a href="$Link" class="button hollow secondary button--flex-full button--skinny"><span class="topicholder-cat-inner" style="<% if $JobListings %>font-weight: bold;<% end_if %>">$Title&nbsp;<span style="topicholder-cat-inner__count">({$JobListings.Count})</span></span></a></div>
        <% end_loop %>
        <% loop $Departments.Sort('Title ASC') %>
            <div class="topic-browse-by-filter__item topic-browse-by-filter__item-small margin-bottom-1" data-equalizer-watch><a href="$Link" class="button hollow secondary button--flex-full button--skinny"><span class="topicholder-cat-inner">$Title&nbsp;<span style="topicholder-cat-inner__count">({$JobListings.Count})</span></span></a></div>
          <% end_loop %>
        </div>
      <% end_if %>
